...
def value_iteration(vision, n_tongs):
    """Run value iteration algorithm to get the Q-value.
    ...
    for _ in range(500):
        ...
        for px in range(11):
            for py in range(11):
                ...
                # Do not update for locations collecting items, to avoid endless loops.
                if (item == 'diamond' and n_tongs > 0) or item == 'tongs' \
                        or item == 'jellybean' or item == 'wall':
                    continue
                for d in range(4):
                    for a in range(3):
